...
export default function PropertyList() {
  ...
  const [contactType, setContactType] = useState<"email" | "call" | "whatsapp">(
    "email"
  );
  ...
  console.log("gdggd",filteredProperties);
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-2 sm:px-4 py-4 sm:py-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3">
                      ...
                      <Button
                        ...
                        onClick={() => handleWhatsappClick(property?.owner?.phone)}
                      >
                        <img
                          className="h-7 w-7"
      ...
      {/* Add pagination UI after property grid */}
      <div className="mt-8 flex justify-center gap-2">
        {Array.from({ length: metadata.totalPages }, (_, i) => i + 1).map(
          (page) => (
            <Button
              key={page}
              variant={page === metadata.page ? "default" : "outline"}
              size="sm"
      ...
      <div className="mt-4 text-center text-sm text-gray-600">
        ...
        {Math.min(metadata.page * metadata.limit, metadata.count)} of{" "}
        {metadata.count} properties
      </div>
    </div>
  );
}
